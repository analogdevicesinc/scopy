...
def get_args():
    ...
    parser.add_argument("--add_submodule", help="Add submodue")
    parser.add_argument("--style", help="Add style templates", action='store_true')
    parser.add_argument("--init", help="Init package.", action='store_true')
    ...
    parser.add_argument("--dest", help="Path to the destination directory for the archive. If not specified, the archive will be created in the current directory")
...
def generate_pkg(packages_path, config, args):
    ...
    if args.plugin or args.all:
        if isinstance(args.plugin, str):
            ...
            plugin_config_path = os.path.join(scopy_path, "tools/plugingenerator/plugin.json")
