...
## Qt6 build process for Scopy
## Based on ubuntu_build_process.sh, targeting Qt 6.7.0
    ...
    if [ "$ID" = "ubuntu" ]; then
        case "$VERSION_ID" in
            ...
            "24.04")
                ...
                ;;
            *)
                echo "Running on Ubuntu, but not 22.04/24.04 (detected: $VERSION_ID)"
                ;;
        esac
    else
...
install_qt() {
	...
	sudo python3 -m aqt install-qt --outputdir $QT_INSTALL_LOCATION linux desktop 6.7.0 -m qt3d qtscxml
}
