...
rm -f ${PACKAGE}_${VERSION}-1.dsc
rm -f ${PACKAGE}_${VERSION}-1_${ARCHITECTURE}.deb
rm -f ${PACKAGE}_${VERSION}-1_${ARCHITECTURE}.buildinfo
rm -f ${PACKAGE}_${VERSION}-1_${ARCHITECTURE}.changes

# Create working directory from local checkout (excluding .git)
echo "Creating working directory from local checkout..."
rsync -a --exclude='.git' "${REPO_ROOT}/" "${WORK_DIR}/"
