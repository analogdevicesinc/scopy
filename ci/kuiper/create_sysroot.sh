...
install_packages(){
	sudo apt install git wget unzip python3
}

download_kuiper(){
	mkdir -p ${STAGING_AREA}
	pushd ${STAGING_AREA}
	wget ${KUIPER_DOWNLOAD_LINK}
	unzip image*.zip
	popd
}
...
apt -y install build-essential gcc g++ gdb-multiarch cmake autoconf automake bison flex git wget pkg-config figlet gawk unzip libsndfile1-dev \
	...
	libxcb-present-dev libxcb-record0-dev libxcb-res0-dev libxcb-xrm-dev \
	libxcb-screensaver0-dev libxcb-util0-dev libxcb-xf86dri0-dev libxcb-xtest0-dev
apt -y install libunwind-dev libsndfile1-dev mesa-utils* mesa-common-dev libglu1-mesa-dev freeglut3-dev mesa-common-dev
