...
set_config_opts() {
	...
	# CONFIG_OPTS+=("--build=x86_64-pc-linux-gnu")
	CONFIG_OPTS+=("--host=${TOOLCHAIN_HOST}")
	CONFIG_OPTS+=("--with-sysroot=${SYSROOT}")
	CONFIG_OPTS+=("--libdir=${SYSROOT}/usr/lib/${TOOLCHAIN_HOST}")
...
build_iio-emu(){
	...
	CURRENT_BUILD_CMAKE_OPTS="-DCMAKE_EXE_LINKER_FLAGS=-lz -licuuc -licudata -llzma" # ???
	build_with_cmake
	sudo make install
	popd
	popd
