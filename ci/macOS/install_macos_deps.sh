...
setup_git_cache() {
    ...
    if [ "${GIT_REPOS_CACHE_RESTORED}" = "true" ]; then
        echo "✅ Git repositories cache restored"
        # Validate cache isn't corrupted
        if [ -d "$GIT_CACHE_DIR" ] && [ "$(ls -A $GIT_CACHE_DIR 2>/dev/null)" ]; then
            export GIT_CACHE_ENABLED=true
        else
            echo "⚠️  Git cache directory empty or corrupted - clearing and rebuilding"
            rm -rf "$GIT_CACHE_DIR"
            mkdir -p "$GIT_CACHE_DIR"
            export GIT_CACHE_ENABLED=false
        fi
    else
        echo "ℹ️  No Git cache - cloning fresh repositories"
        export GIT_CACHE_ENABLED=false
    fi
}
...
install_packages() {
	...
	# Smart package installation based on cache status
	if [ "${HOMEBREW_CACHE_RESTORED}" = "true" ]; then
	...
	# Keep current package list logic exactly the same
	macos_version=$(sw_vers -productVersion)
	major_version=$(echo "$macos_version" | cut -d '.' -f 1)
	if [ "$major_version" -gt 12 ]; then
...
clone() {
	...
	if [ "$GIT_CACHE_ENABLED" = "true" ]; then
		...
		# Simple check: if cache directory has content, use it
		if [ -d "$GIT_CACHE_DIR" ] && [ "$(ls -A $GIT_CACHE_DIR 2>/dev/null)" ]; then
			echo "✅ Copying cached repositories"
			cp -r "$GIT_CACHE_DIR"/* .

			# Update submodules for all repos
			for repo_dir in libserialport libiio libad9361 libm2k gr-scopy gr-m2k gnuradio qwt libsigrokdecode libtinyiiod KDDockWidgets extra-cmake-modules karchive; do
				if [ -d "$repo_dir" ]; then
					cd "$repo_dir"
			...
			echo "⚠️  Git cache directory empty - cloning fresh"
			export GIT_CACHE_ENABLED=false
		fi
	fi
	...
	if [ "$GIT_CACHE_ENABLED" = "false" ]; then
		...
		# Original clone method (bash 3.2 compatible)
		...
		if [ -n "$GIT_CACHE_DIR" ]; then
			mkdir -p "$GIT_CACHE_DIR"
			cp -r libserialport libiio libad9361 libm2k gr-scopy gr-m2k gnuradio qwt libsigrokdecode libtinyiiod KDDockWidgets extra-cmake-modules karchive "$GIT_CACHE_DIR/"
		fi
	fi
...
build_deps(){
	if [ "$DEPENDENCIES_CACHED" = "true" ]; then
		echo "✅ Using cached dependencies - skipping build"
		echo "📦 Found cached dependencies in $STAGING_AREA_DEPS"
		return 0
	fi
