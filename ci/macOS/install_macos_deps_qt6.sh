...
# Usage: ./install_macos_deps_qt6.sh

set -ex
REPO_SRC=$(git rev-parse --show-toplevel)
source $REPO_SRC/ci/macOS/macos_config_qt6.sh
...
install_qt() {
	...
	python3 -m aqt install-qt --outputdir $QT_INSTALL_LOCATION mac desktop 6.8.3 clang_64 -m qt3d qtscxml
}
...
export_paths(){
	...
	CMAKE_OPTS="-DCMAKE_PREFIX_PATH=$QT\;$STAGING_AREA_DEPS -DCMAKE_INSTALL_PREFIX=$STAGING_AREA_DEPS -DCMAKE_POLICY_VERSION_MINIMUM=3.5"
