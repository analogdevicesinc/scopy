...
QwtInterval WaterfallData::interval(Qt::Axis axis) const
{
	...
	case Qt::YAxis: return QwtInterval(0.0, static_cast<double>(m_maxRows));
...
double WaterfallData::value(double x, double y) const
{
	...
	const int bin = static_cast<int>((x - m_xInterval.minValue()) / xRange * static_cast<double>(m_fftSize - 1) + 0.5);
