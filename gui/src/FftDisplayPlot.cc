							     ...
							     bool history_en)
{
	...
	case PEAK_HOLD_CONTINUOUS:
		return boost::make_shared<PeakHoldContinuous>(data_width, history);
	case MIN_HOLD:
		return boost::make_shared<MinHold>(data_width, history);
	case MIN_HOLD_CONTINUOUS:
		return boost::make_shared<MinHoldContinuous>(data_width, history);
	case LINEAR_RMS:
		if (history_en) {
			return boost::make_shared<LinearRMS>(data_width, history);
		} else {
			return boost::make_shared<LinearRMSOne>(data_width, history);
		}
	case LINEAR_DB:
		if (history_en) {
			return boost::make_shared<LinearAverage>(data_width, history);
		} else {
			return boost::make_shared<LinearAverageOne>(data_width, history);
		}
	case EXPONENTIAL_RMS:
		return boost::make_shared<ExponentialAverage>(data_width, history);
	case EXPONENTIAL_DB:
		return boost::make_shared<ExponentialAverage>(data_width, history);
	default:
		return nullptr;
