...
void BasicPlot::start() {
	if(debug) qDebug(CAT_GUI_PLOT)<<QString::number(id)<<"BasicPlot - Starting plot - framerate - " << replotFrameRate;
	pfps.clearHistory();
	ifps.clearHistory();
	ims.clearHistory();
	pms.clearHistory();
...
void BasicPlot::stop() {
	if(debug) qDebug(CAT_GUI_PLOT)<<QString::number(id)<<"BasicPlot - Stopping plot - will force one replot";
	started = false;
	replotTimer.stop();
	replotNow();
}
...
//	auto instrumentFpsAvg = ifps.pushValueReturnAverage(instrumentFps);
	...
	if(debug) qDebug(CAT_GUI_PLOT)	<< QString::number(id) << " - BasicPlot - drawing plot - "
...
void BasicPlot::replot() {
		...
		{
			if(debug) qDebug(CAT_GUI_PLOT)<<QString::number(id)<<"BasicPlot - replot - already scheduled";
		}
#else
	replotNow();
#endif
