...
QString PlotAPI::getTitle()
{
	return m_component ? m_component->name() : QString();
}
...
bool PlotAPI::isLabelsEnabled()
{
	return m_plot ? m_plot->showXAxisLabels() : false;
}
...
double PlotAPI::getXMin()
{
	return m_plot && m_plot->xAxis() ? m_plot->xAxis()->min() : 0.0;
}

double PlotAPI::getXMax()
{
	return m_plot && m_plot->xAxis() ? m_plot->xAxis()->max() : 0.0;
}
...
double PlotAPI::getYMin()
{
	return m_plot && m_plot->yAxis() ? m_plot->yAxis()->min() : 0.0;
}

double PlotAPI::getYMax()
{
	return m_plot && m_plot->yAxis() ? m_plot->yAxis()->max() : 0.0;
}
