...
void AnnotationCurve::annotationCallback(srd_proto_data* pdata, void* annotationCurve)
{
	...
	(*row_iter).second.emplaceAnnotation(pdata, &((*row_iter).first));
...
void AnnotationCurve::sortRows()
{
	for (auto it = m_annotationRows.begin(); it != m_annotationRows.end(); ++it) {
		it->second.sortAnnotations();
	}
}

void AnnotationCurve::newAnnotations() { QMetaObject::invokeMethod(plot(), "replot"); }
...
QWidget* AnnotationCurve::getCurrentDecoderStackMenu()
{
	...
	for (auto& ch : channels) {
		...
		chls->setLayout(lay);
		QString required = (ch->isOptional ? "" : "*");
		...
		if (!ch->assignedSignal) {
			...
			box->setCurrentIndex(ch->bitId + 1);
		}
...
std::vector<std::shared_ptr<scopy::bind::Decoder>> AnnotationCurve::getDecoderBindings() { return m_bindings; }
				...
				const QRectF& canvasRect, int from, int to) const
{
	...
	for (int row = 0; row < m_annotationRows.size(); ++row) {
		...
		std::tie(start, stop) = (*it).second.getAnnotationSubset(fromTimeToSample(interval.minValue()),
		...
		for (; start <= stop; ++start) {
			...
			const double annotation_width = xMap.transform(fromSampleToTime(ann.endSample())) -
				xMap.transform(fromSampleToTime(ann.startSample()));
			...
			const double delta = (previous_end != -1 ? xMap.transform(fromSampleToTime(ann.endSample())) -
			...
			if (shouldDraw) {
				drawAnnotation(currentRowOnPlot, ann, painter, xMap, yMap, canvasRect, mapper, size);
				previous_end = -1;
				annotations_in_block = 0;
			} else {
				previous_end = ann.endSample();
				prev_ann = ann;

				if (!annotations_in_block) {
					block_start = ann.startSample();
				}
				     ...
				     const QSizeF& titleSize) const
{
	...
	if (ann.startSample() != ann.endSample()) {
		drawTwoSampleAnnotation(row, ann, painter, xMap, yMap, canvasRect, mapper, titleSize);
	} else {
		drawOneSampleAnnotation(row, ann, painter, xMap, yMap, canvasRect, mapper, titleSize);
	}
}

void AnnotationCurve::fillCurve(int row, uint32_t annClass, QPainter* painter, const QwtScaleMap& xMap,
					      ...
					      const QSizeF& titleSize) const
{
	...
	double first = fromSampleToTime(ann.startSample());
	double last = fromSampleToTime(ann.endSample());
	...
	fillCurve(row, ann.annClass(), painter, xMap, yMap, canvasRect, polyline);
...
void AnnotationCurve::drawOneSampleAnnotation(int row, const Annotation& ann, QPainter* painter,
					      ...
					      const QSizeF& titleSize) const
{
	double xx = xMap.transform(fromSampleToTime(ann.startSample()));
	// 20 px
	double HeightInPoints = yMap.invTransform(m_traceHeight) - yMap.invTransform(0);
	...
	QBrush br = brush();
	QColor color;
	const int h = (55 * ann.annClass()) % 360;
	const int s = 180;
	const int v = 170;
	color.setHsl(h, s, v);
	br.setColor(color);
	...
	double x1 = fromSampleToTime(ann.startSample()) - WidthInPoints / 2.0;
	double x2 = fromSampleToTime(ann.startSample()) + WidthInPoints / 2.0;
