...
std::vector<std::shared_ptr<Decoder>> AnnotationDecoder::getDecoderStack() { return m_stack; }
...
void AnnotationDecoder::assignChannel(uint16_t chId, uint16_t bitId)
{
	...
	for (auto& ch : m_channels) {
		if (ch.id == chId) {
			ch.bitId = bitId;
			ch.assignedSignal = true;
			break;
		}
	}

	for (auto& ch : m_channels) {
		qDebug() << "ch: " << ch.id << " has ch assigned: " << ch.assignedSignal << " with bitid: " << ch.bitId;
	}
...
void AnnotationDecoder::stackChanged()
{
	...
	m_stack.front()->setChannels(chls);

	srd_decoder_inst* prev_di = nullptr;
	for (const shared_ptr<logic::Decoder>& dec : m_stack) {
		srd_decoder_inst* const di = dec->createDecoderInst(m_srdSession);
		if (prev_di)
