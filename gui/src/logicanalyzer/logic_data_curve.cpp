...
LogicDataCurve::LogicDataCurve(uint16_t* data, uint8_t bit, scopy::logic::LogicTool* logic)
			       ...
			       const QRectF& canvasRect, int from, int to) const
{
	...
	if (edges.back().first + 1 < m_endSample - 1) {
		displayedData += QPointF(fromSampleToTime(m_endSample - 1),
					 (!edges.back().second) * heightInPoints + m_pixelOffset);
	}
