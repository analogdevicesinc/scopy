...
void waterfall_sink_impl::resize_bufs(int size)
{
	if(size != fft_size()) {
		...
		for(int n = 0; n < d_nconnections; n++) {
			...
			d_buffers[n] = (double*)volk_malloc(d_fftsize*sizeof(double),
							    volk_get_alignment());
			memset(d_buffers[n], 0, d_fftsize*sizeof(double));

			volk_free(d_fbuffers[n]);
			d_fbuffers[n] = (gr_complex*)volk_malloc(d_fftsize*sizeof(gr_complex),
							    volk_get_alignment());
	...
	if((d_end != 0 && !d_displayOneBuffer) ||
		...
		for(n = 0; n < d_nconnections; n++) {
			...
			for (int x = 0; x < fft_size(); x++) {
				d_buffers[n][x] = (double)((1.0 - d_fftavg) * d_buffers[n][x] +
							   (d_fftavg)*d_fbuf[x]);
			}
		}
		...
		if((gr::high_res_timer_now() - d_last_time > d_update_time) || !d_cleanBuffers) {
			...
			d_qApplication->postEvent(d_main_gui, new WaterfallUpdateEvent(d_buffers, nItemsToSend, gr::high_res_timer_now() - d_last_time));
		}
