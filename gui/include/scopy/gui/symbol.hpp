...
protected:
	QRect m_surface;
	QPoint m_anchor; // sets which point of d_surface to point exactly to d_plotCoord. Default point is: top-left of
			 // d_surface (d_anchor = (0, 0))
	QPointF m_plotCoord; // Position of symbol in plot coordinates
	bool m_selected;

	QwtAxisId m_fixedAxis;	// The axis on which the symbol must not move
	QwtAxisId m_mobileAxis; // The axis on which the symbol can be shifted
	double m_stepSize; // Symbol position can take only values that are a multiple of d_stepSize. If d_stepSize = 0
			   // symbol pos can take any value.
	bool m_oppToFixed; // If symbol should be placed diametrically opposed to the fixed axis
	bool m_floats;	   // If symbols floats on top of the plot or is attached to the plot
	bool m_withinPlot; // If symbol position can only be within the plot visible area
