    ...
    ui(new Ui::BitfieldWidget())
{
    ...
    connect(ui->valueSpinBox, SIGNAL(valueChanged(int)), this, SLOT(setValue(int))); //connect spinBox singnal to the value changed signal
}
...
//        label->setContentsMargins(0, 0, 10, 0);
    ...
    else
    {
        ...
        ui->valueComboBox->addItem(temp.firstChildElement("Description").text());
         ...
         connect(ui->valueComboBox,SIGNAL(currentIndexChanged(int)), this, SLOT(setValue(int))); //connect comboBox signal to the value changed signal
    }
}
