...
OscCustomScrollArea::OscCustomScrollArea(QWidget *parent)
	: QScrollArea(parent), inside(false), disableCursor(true) {
	QScroller::grabGesture(this->viewport(),
			       QScroller::LeftMouseButtonGesture);

	scroll = QScroller::scroller(this->viewport());

	QScrollerProperties properties =
		QScroller::scroller(this->viewport())->scrollerProperties();
	...
	QScroller::scroller(this->viewport())
		->setScrollerProperties(properties);

	connect(scroll, &QScroller::stateChanged,
