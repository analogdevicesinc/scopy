...
############################################################################
# Check if the files modified in the current pull request respect
# the coding style defined in the .clang-format file
############################################################################
check_clangformat() {

    git diff --name-only --diff-filter=d origin/${TARGET_BRANCH} FETCH_HEAD | while read -r file; do
    # git ls-tree -r --name-only HEAD | while read -r file; do  # performs check over the whole project
...
check_cmakeformat() {
    find . -type f -iname CMakeLists.txt -o -type f -iname "*.cmake" | xargs /home/vsts/.local/bin/cmake-format -i
