...
class TST_RFPowerMeterPlugin : public QObject
{
	Q_OBJECT
private Q_SLOTS:
	void fileExists();
...
void TST_RFPowerMeterPlugin::isLibrary() { QVERIFY(QLibrary::isLibrary(FILENAME)); }

void TST_RFPowerMeterPlugin::className()
{
	QPluginLoader qp(FILENAME, this);
	QVERIFY(qp.metaData().value("className") == "RFPowerMeterPlugin");
}

void TST_RFPowerMeterPlugin::loaded()
{
	QPluginLoader qp(FILENAME, this);
	qp.load();
	QVERIFY(qp.isLoaded());
...
void TST_RFPowerMeterPlugin::instanceNotNull()
{
	QPluginLoader qp(FILENAME, this);
	QVERIFY(qp.instance() != nullptr);
}

void TST_RFPowerMeterPlugin::multipleInstances()
{
...
void TST_RFPowerMeterPlugin::qobjectcast_to_plugin()
{
	QPluginLoader qp(FILENAME, this);
...
void TST_RFPowerMeterPlugin::clone()
{
	QPluginLoader qp(FILENAME, this);
...
void TST_RFPowerMeterPlugin::name()
{
	QPluginLoader qp(FILENAME, this);
...
void TST_RFPowerMeterPlugin::metadata()
{
	QPluginLoader qp(FILENAME, this);
...
QTEST_MAIN(TST_RFPowerMeterPlugin)
