...
DataWriter::DataWriter(bool withHeader, QObject *parent)
	: QObject(parent), m_withHeader(withHeader)
{}
...
QFileInfo DataWriter::getFileInfo()
{
	return QFileInfo(m_file);
}

void DataWriter::setWithHeader(bool withHeader)
{
	m_withHeader = withHeader;
}

bool DataWriter::hasHeader() const
{
	return m_withHeader;
}

void DataWriter::setIQHeader(const IQBinHeader &header)
{
	m_iqHeader = header;
}

IQBinHeader DataWriter::getIQHeader() const
{
	return m_iqHeader;
}
