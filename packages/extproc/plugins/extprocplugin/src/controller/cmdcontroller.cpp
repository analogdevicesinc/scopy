...
Q_LOGGING_CATEGORY(CAT_QIQ_CONTROLLER, "CMDController");
...
ProcConfiguration *CMDController::getCurrentConfig() { return m_qiqConfig; }

bool CMDController::isReady() { return m_qiqConfig->isComplete(); }
...
QString CMDController::findCli()
{
	...
	cliPath = buildCliPath(Preferences::get("qiq_cli_path").toString());
	if(!cliPath.isEmpty() && QFile::exists(cliPath)) {
		qInfo(CAT_QIQ_CONTROLLER) << "cli path from preferences:" << cliPath;
		return cliPath;
	}
	// Search the cli next to scopy executable.
	cliPath = buildCliPath(scopy::config::executableFolderPath());
	if(!cliPath.isEmpty() && QFile::exists(cliPath)) {
		qInfo(CAT_QIQ_CONTROLLER) << "cli path:" << cliPath;
		return cliPath;
	}
