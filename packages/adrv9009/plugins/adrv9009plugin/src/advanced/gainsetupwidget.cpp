...
GainSetupWidget::GainSetupWidget(component::Device *device, IIOWidgetGroup *group, QWidget *parent)
	: QWidget(parent)
	, m_device(device)
	, m_widgetGroup(group)
{
...
QWidget *GainSetupWidget::createRxGainSection(QWidget *parent)
{
	...
	QMap<QString, QString> gainModeOptions;
...
QWidget *GainSetupWidget::createObservationGainSection(QWidget *parent)
{
	...
	obsGainSection->contentLayout()->addWidget(widget);
	Style::setBackgroundColor(widget, json::theme::background_primary);
	Style::setStyle(widget, style::properties::widget::border_interactive);
