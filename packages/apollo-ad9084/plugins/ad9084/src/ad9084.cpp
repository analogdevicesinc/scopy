...
Ad9084::Ad9084(struct iio_device *dev, IIOWidgetGroup *mgr, QWidget *parent)
	: QWidget(parent)
	, m_mgr(mgr)
	, m_device(dev)
	, m_channelPaths({})
...
void Ad9084::scanChannels()
{
	...
	for(unsigned int i = 0; i < m_rx_coarse_ddc_channel_names.size(); i++) {
		...
		Ad9084Channel *rxchnWidget = new Ad9084Channel(rxchn, i, m_mgr, this);
	...
	for(unsigned int i = 0; i < m_tx_coarse_duc_channel_names.size(); i++) {
		...
		Ad9084Channel *txchnWidget = new Ad9084Channel(txchn, i, m_mgr, this);
