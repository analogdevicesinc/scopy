...
bool Fmcomms11Plugin::compatible(QString m_param, QString category)
{
	...
	bool hasAdc = iio_context_find_device(conn->context(), "axi-ad9625-hpc") != nullptr;
	bool hasDac = iio_context_find_device(conn->context(), "axi-ad9162-hpc") != nullptr;
	ret = hasAdc && hasDac;
...
bool Fmcomms11Plugin::onConnect()
{
	...
	Fmcomms11Tool *tool = new Fmcomms11Tool(conn->context(), m_widgetGroup);
