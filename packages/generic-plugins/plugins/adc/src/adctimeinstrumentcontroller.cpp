...
void ADCTimeInstrumentController::createTimeSink(AcqTreeNode *node)
{
	...
	connect(c, &GRTimeSinkComponent::arm, this, &ADCTimeInstrumentController::onStart);
	connect(c, &GRTimeSinkComponent::disarm, this, &ADCTimeInstrumentController::onStop);

	connect(c, SIGNAL(ready()), this, SLOT(startUpdates()));
	connect(c, SIGNAL(finish()), this, SLOT(stopUpdates()));
	connect(c, &GRTimeSinkComponent::requestForceStop, this, &ADCTimeInstrumentController::stop);
}
