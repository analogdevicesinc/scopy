	...
	, QWidget{parent}
{
	...
	mainLayout->addWidget(plotTimeAxisController);
...
{
	MenuSectionWidget *editTitleContainer = new MenuSectionWidget(parent);
	MenuCollapseSection *editTitleSection = new MenuCollapseSection(
		"PLOT TITLE", MenuCollapseSection::MHCW_NONE, MenuCollapseSection::MHW_BASEWIDGET, editTitleContainer);

	editTitleContainer->contentLayout()->addWidget(editTitleSection);

	QLineEdit *titleEdit = new QLineEdit(m_plot->name(), parent);
	connect(titleEdit, &QLineEdit::textChanged, this, [=](const QString &text) { m_plot->setName(text); });

	editTitleSection->contentLayout()->addWidget(titleEdit);

	return editTitleContainer;
}
