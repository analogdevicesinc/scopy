...
QList<DmmDataMonitorModel *> DMM::getDmmMonitors(iio_context *ctx)
{
	...
	auto deviceCount = IIOContext::get_devices_count(ctx);
	for(int i = 0; i < deviceCount; i++) {
		iio_device *dev = IIOContext::get_device(ctx, i).data();
		int chnCout = IIODevice::get_channels_count(dev);
		for(int j = 0; j < chnCout; j++) {
			...
			if(isDMMCompatible(chn) || isHwmon(dev, chn)) {
				QString name =
					QString(IIODevice::get_name(dev)) + ":" + QString(IIOChannel::get_id(chn));

				UnitOfMeasurement *unitOfMeasurement = new UnitOfMeasurement("", "");
				DMMReadStrategy *dmmReadStrategy = new DMMReadStrategy(dev, chn);

				IIOResult<const iio_attr *> attr = IIOChannel::find_attr(chn, "offset");
				if(attr.ok()) {
					double offset = 0;
					IIOAttribute::read(attr.data(), &offset);
					dmmReadStrategy->setOffset(offset);
				}

				iio_chan_type type = IIOChannel::get_type(chn);
				if(type != iio_chan_type::IIO_CHAN_TYPE_UNKNOWN) {
					IIOUnit dmmInfo = m_iioDevices.value(type);
...
bool DMM::isDMMCompatible(iio_channel *chn)
{
	...
	if(!IIOChannel::is_output(chn) && iioChannelHasAttribute(chn, "raw") && iioChannelHasAttribute(chn, "scale")) {
		return true;
	}
	return false;
}
...
{
	if(IIODevice::is_hwmon(dev) && iioChannelHasAttribute(chn, "input")) {
		auto d_name = IIODevice::get_name(dev);
		auto c_name = std::string(IIOChannel::get_id(chn));
...
{
	IIOResult<const iio_attr *> res = IIOChannel::find_attr(chn, attr.c_str());
	return res.ok();
}
