...
#include <QMainWindow>
#include "src/refactoring/maingui/channel_widget.hpp"
#include "src/refactoring/maingui/menu_header.hpp"
        ...
        m_ui(new Ui::ToolView)
{
	...
	m_centralMainWindow->setCentralWidget(nullptr);
	...
	connect(settingsBtn, &QPushButton::clicked, this, [=, this](bool checked) {
		if (!m_menuOrder.isEmpty()) {
			...
			if (checked) {
				...
				btn = dynamic_cast<CustomPushButton*>(m_group.checkedButton());
			}
...
void ToolView::rightMenuFinished(bool opened)
{
	...
	while (!m_menuButtonActions.empty()) {
...
void ToolView::triggerRightMenuToggle(bool checked)
{
	...
	// finishes
	auto* btn = dynamic_cast<CustomPushButton*>(QObject::sender());
