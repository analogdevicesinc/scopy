...
uint32_t MonitorPlotManager::addPlot(QString name)
{
	...
	m_plotIdx++;
	m_plots.append(plt);
	// the buffer previewer follows the main plot
	if(m_primary == nullptr) {
		m_primary = plt;
		m_primary->generateBufferPreviewer(m_bufferpreviewer);
		m_plotpreviewer = m_primary->bufferPreviewer();
		int idx = m_lay->indexOf(m_statsPanel);
		...
		if(plt) {
			QObject::connect(plt, &MonitorPlot::nameChanged, combo, [combo, uuid](const QString &newName) {
				for(int j = 0; j < combo->count(); ++j) {
					if(combo->itemData(j).toUInt() == uuid) {
