...
DebuggerInstrument::DebuggerInstrument(struct iio_context *ctx, QJSEngine *engine,
		   QWidget *parent) :
	QWidget(parent),
	...
	, m_toolView(nullptr)
{
	...
	QObject::connect(ui->DevicecomboBox, &QComboBox::currentTextChanged, this,
			 &DebuggerInstrument::updateSources);

	QObject::connect(ui->addressSpinBox, SIGNAL(valueChanged(int)), this,
			 SLOT(updateRegMap()));

	QObject::connect(ui->valueRegisterSpinBox, SIGNAL(valueChanged(int)), this->reg,
			 SLOT(setValue(int)));
	QObject::connect(this->reg, SIGNAL(valueChanged(int)), ui->valueRegisterSpinBox,
			 SLOT(setValue(int)));
