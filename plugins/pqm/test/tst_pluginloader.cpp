...
class TST_PQMPlugin : public QObject
{
	Q_OBJECT
private Q_SLOTS:
	void fileExists();
...
void TST_PQMPlugin::isLibrary() { QVERIFY(QLibrary::isLibrary(FILENAME)); }

void TST_PQMPlugin::className()
{
	QPluginLoader qp(FILENAME, this);
	QVERIFY(qp.metaData().value("className") == "PQMPlugin");
}

void TST_PQMPlugin::loaded()
{
	QPluginLoader qp(FILENAME, this);
	qp.load();
	QVERIFY(qp.isLoaded());
...
void TST_PQMPlugin::instanceNotNull()
{
	QPluginLoader qp(FILENAME, this);
	QVERIFY(qp.instance() != nullptr);
}

void TST_PQMPlugin::multipleInstances()
{
...
void TST_PQMPlugin::qobjectcast_to_plugin()
{
	QPluginLoader qp(FILENAME, this);
