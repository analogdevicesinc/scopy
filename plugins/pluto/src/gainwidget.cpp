...
QWidget *GainWidget::modeWidget(QWidget *parent)
{
	...
	rx1Mode->setToolTip("RX1 Gain control operation: Manual gain (0); Fast attack AGC (1); Slow attack AGC (2); Hybrid AGC (3). See register 0x0FA, bits [D4], [D1:D0].");
	...
	rx2Mode->setToolTip("RX2 Gain control operation: Manual gain (0); Fast attack AGC (1); Slow attack AGC (2); Hybrid AGC (3). See register 0x0FA, bits [D4], [D3:D2].");
	...
	decpowMeasurement->setToolTip("The power measurement duration used by the gain control algorithm. See register 0x15C, bits [D3:D0]. This data is processed by the driver.");
	...
	lowPowerThresh->setToolTip("This threshold is used by the fast AGC to determine if the gain should be increased. It can also be used to trigger a CTRL_OUT signal transition in MGC mode. See register 0x114, bits [D6:D0]. This data is processed by the driver.");
	...
	attackDelay->setToolTip("The AGC Attack Delay prevents the AGC from starting its algorithm until the receive path has settled. The delay counter starts when the ENSM enters the Rx state. See register 0x022, bits [D5:D0]. This data is processed by the driver.");
	...
	rxFirOut->setToolTip("	Set to use the RX FIR output for power measurements. Default/Clear to use the HB1 output. See register 0x15C, bits [D6].");
...
QWidget *GainWidget::mgcWidget(QWidget *parent)
{
	...
	mgcRx1Ctrl->setToolTip("If this attribute is clear, SPI writes change the RX1 gain. When this attribute is set, control input pins control the gain. See register 0x0FB, bit [D0].");
	...
	mgcRx2Ctrl->setToolTip("If this attribute is clear, SPI writes change the RX2 gain. When this attribute is set, control input pins control the gain. See register 0x0FB, bit [D1].");
	...
	mgcIncGain->setToolTip("This attribute applies if the CTRL_IN signals control gain. The gain index increases by this value when certain CTRL_IN signals transition high. See register 0x0FC, bits [D7:D5]. This data is processed by the driver.");
	...
	mgcDecGain->setToolTip("This attribute applies if the CTRL_IN signals control gain. The gain index decreases by this value when certain CTRL_IN signals transition high. See register 0x0FE, bits [D7:D5]. This data is processed by the driver.");
	...
	mgcSplitTableCtrl->setToolTip("AGC determine this (0); Only in LPF(1); Only in LMT (2). See register 0x0FC, bits [D4], [D3].");
...
QWidget *GainWidget::agcThresholdGainChangesWidget(QWidget *parent)
{
	...
	outerThreshHigh->setToolTip("The outer high threshold equals the inner high threshold plus this value. See register 0x129, bits [D7:D4]. This data is processed by the driver.");
	...
	outerThreshHighDecSteps->setToolTip("The slow AGC changes gain by this amount when the outer high threshold is exceeded. See register 0x12A, bits [D7:D4].");
	...
	innerThreshHigh->setToolTip("	Applies to AGC. This attribute specifies the fast AGC lock level or specifies the slow AGC inner high threshold. Resolution is −1 dBFS/LSB. See register 0x101 [D6:D0]).");
	...
	innerThreshHighDecSteps->setToolTip("This attribute sets the gain decrease amount when the inner high threshold is exceeded. See register 0x123, bits [D6:D4].");
	...
	innerThreshLow->setToolTip("This attribute sets the slow AGC inner low window threshold. See register 0x120, bits [D6:D0].");
	...
	innerThreshLowIncSteps->setToolTip("This attribute sets the increase amount used when the gain goes under the inner low threshold. See register 0x123, bits [D2:D0].");
	...
	outerThreshLow->setToolTip("The outer low threshold equals the inner low threshold plus this value. See register 0x129, bits [D3:D0]. This data is processed by the driver.");
	...
	outerThreshLowIncSteps->setToolTip("	The slow AGC changes gain by this amount when the outer low threshold is exceeded. See register 0x12A, bits [D3:D0].");
	...
	sync->setToolTip("If this attribute is set, CTRL_IN2 transitioning high resets the counter.See register 0x128, bit D4.");
...
QWidget *GainWidget::adcOverloadWidget(QWidget *parent)
{
	...
	largeOverloadExceed->setToolTip("This counter specifies the number of large ADC overloads that must occur before the gain will decrease by the large ADC overload gain step. See register 0x122, bits [D7:D4].");
	...
	smallOverloadExceed->setToolTip("This counter specifies the number of small ADC overloads that must occur to prevent a gain increase. See register 0x122, bits [D3:D0].");
	...
	largeDecSteps->setToolTip("	This attribute applies to AGC and determine how much the gain changes for large LPF in split tablemode or the large LMT and large ADC overloads in full table mode. See register 0x106, bits [D3:D0]");
	...
	preventGainIncrease->setToolTip("This attribute set the slow AGC inner low window threshold. See register 0x120, bits [D6:D0].");
	...
	immedGainChange->setToolTip("Set this attribute to allow large ADC overload to reduce gain immediately. See register 0x123, bit D3.");
...
QWidget *GainWidget::lmtOverloadWidget(QWidget *parent)
{
	...
	highThresh->setToolTip("This attribute sets the large LMT overload threshold. See register 0x108. This data is processed by the driver.");
	...
	lowThresh->setToolTip("This attribute sets the small LMT overload threshold. See register 0x107. This data is processed by the driver.");
	...
	largeExced->setToolTip("This counter specifies the number of large LMT overloads that must occur before gain decreases by the LMT Gain Step. See register 0x121, bits [D7:D4].");
	...
	smallExced->setToolTip("This counter specifies the number of small LMT overloads that much occur to prevent a gain increase. See register 0x121, bits [D3:D0].");
	...
	largeInc->setToolTip("	This attribute determines how much the gain changes for large LMT in split tablemode or the small ADC overload for the full table. See register 0x103, bits [D4:D2].");
	...
	immedGain->setToolTip("Set this attribute to allow large LMT overloads to reduce gain immediately. See register 0x123, bit D7.");
...
QWidget *GainWidget::digitalGainWidget(QWidget *parent)
{
	...
	gainMode->setToolTip("	This attribute is used in split table mode to enable the digital gain pointer. See register 0x0FB, bit D2.");
	...
	saturationExceedCounter->setToolTip("This counter specifies the number of digital saturation events that much occur to prevent a gain increase. See register 0x128, bits [D3:D0].");
	...
	maxDigGain->setToolTip("This attribute equals the maximum allowable digital gain, and applies to all gain control modes. See register 0x100, bits [D4:D0].");
	...
	gainStepSize->setToolTip("If digital saturation occurs, digital gain reduces by this value. See register 0x100, bits [D7:D5].");
...
QWidget *GainWidget::stateResponseToPeakOverload(QWidget *parent)
{
	...
	stateWaitTime->setToolTip("The fast AGC delays moving from State 1 to State 2 until no peak overloads are detected for the value of this counter; measured in ClkRF cycles. See register 0x117, bits [D4:D0]. This data is processed by the driver.");

	connect(this, &GainWidget::readRequested, this, [=, this](){
		stateWaitTime->readAsync();
	});
...
QWidget *GainWidget::lowPowerCheck(QWidget *parent)
{
	...
	allowGainIncrease->setToolTip("Setting this attribute allows the fast AGC to increase the gain while optimizing the gain index. Clearing it prevents the gain from increasing in any condition. See register 0x110, bit D0.");
	...
	lowPowerThreshIncrementTime->setToolTip("This attribute sets the time that the signal power must remain below the Low Power Threshold before the fast AGC will change gain. Also can be used by the MGC. See register 0x11B, bits [D7:D0].");
	...
	lowPowerThreshIncrementSteps->setToolTip("The Fast AGC will increase the gain index by this amount if signal power decreases below the Low Power Threshold and only if the Enable Incr Gain is enabled. See register 0x117, bits [D7:D5]. This data is processed by the driver.");
...
QWidget *GainWidget::agcLockLevelAdjustment(QWidget *parent)
{
	...
	lockLevelGain->setToolTip("This attribute sets the maximum gain index increase that the fast AGC can use for the lock level adjustment. See register 0x118, bits [D5:D0].");
	...
	lockLevelLmtGain->setToolTip("Set this attribute to allow the AGC to use LMT gain if the gain index needs to increase when moving to the AGC Lock Level. See register 0x111, bit D6.");
...
QWidget *GainWidget::peakDetectors(QWidget *parent)
{
	...
	increaseAfterGainLock->setToolTip("Set this attribute to allow gain increases after the gain has locked but before State 5. Signal power must be lower than the low power threshold for longer than the increment time duration register. See register 0x110, bit D7.");
...
QWidget *GainWidget::gainUnlock(QWidget *parent)
{
	...
	gainIndexType->setToolTip("MAX Gain (0); Optimized Gain (1); Set Gain (2). See register 0x110, bits D4,D2. This data is processed by the driver.");
	...
	increaseAfterGainLock->setToolTip("Set this attribute to use the last gain index of the previous frame for set gain. Clear to use the first gain index of the previous frame. See register 0x111, bit D7.");
	...
	sigThreshExceedEn->setToolTip("If this attribute is set and the fast AGC is in State 5, the gain will not change even if the signal power increase by more than the Stronger Signal Threshold. See register 0x115, bit D7.");
	...
	fagcRstGlaStrongerSigThresh->setToolTip("If the signal power increases by this threshold and the signal power remains at this level or higher for a duration that is twice the Gain Lock Exit Count, the gain may unlock, depending on other AGC configuration bits. See register 0x113, bits [D5:D0].");
	...
	rstGlaEngLostSigThreshExceedEn->setToolTip("If this attribute is set and the fast AGC is in State 5, the gain will not change even if the average signal power decreases more than the Energy Lost Threshold register. See register 0x110, bit D3.");
	...
	rstGlaEnergyLost->setToolTip("If the signal power decreases by this threshold and the signal power remains at this level or lower for a duration that is twice the Gain Lock Exit Count, the gain may unlock, depending on other AGC configuration bits. See register 0x112, bits [D5:D0].");
	...
	rstGlaEnergyLostGotoOptimGainEn->setToolTip("	If this attribute is set and the fast AGC is in State 5, the gain index will go to the optimize gain value if an energy lost state occurs or when the EN_AGC signal goes high. See register 0x110, bit D6.");
	...
	glaLargeAdcOverloadEn->setToolTip("Unlock gain if ADC Ovrg, Lg ADC or LMT Ovrg. See register 0x110, bit D1 and register 0x114, bit D7. This data is processed by the driver.");
	...
	rstGlaIfEnAgcPulled->setToolTip("MAX Gain (0); Optimized Gain (1); Set Gain (2), No gain change. See registers 0x110, 0x111");

	// adi,fagc-power-measurement-duration-in-state5
	IIOWidget *powerMeasurementDuration = IIOWidgetBuilder(widget)
				  .device(m_device)
	...
	powerMeasurementDuration->setToolTip("The power measurement duration used by the gain control algorithm for State 5 (gain lock) - fast AGC. See register 0x109, bit D7 and 0x10a, bits [D7:D5].");
...
QWidget *GainWidget::miscWidget(QWidget *parent)
{
	...
	IIOWidget *stateWaitTime = IIOWidgetBuilder(widget)
					   ...
					   .buildSingle();
	layout->addWidget(stateWaitTime);
	stateWaitTime->setToolTip("The power measurement duration used by the gain control algorithm. See register 0x15C, bits [D3:D0]. This data is processed by the driver.");
