...
class SCOPY_ADMT_EXPORT RegisterBlockWidget : public QWidget {
  Q_OBJECT
public:
  enum ACCESS_PERMISSION { READ, WRITE, READWRITE };

  QPushButton *m_readButton, *m_writeButton;

  RegisterBlockWidget(QString header, QString description, uint32_t address,
                      uint32_t cnvPage,
                      RegisterBlockWidget::ACCESS_PERMISSION accessPermission,
