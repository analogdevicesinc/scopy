...
public:
  ...
  int HAR_MAG_1, HAR_MAG_2, HAR_MAG_3, HAR_MAG_8, HAR_PHASE_1, HAR_PHASE_2,
      HAR_PHASE_3, HAR_PHASE_8, sampleCount = 0;
  ...
  vector<double> angle_errors_fft_pre, angle_errors_fft_phase_pre,
      angle_errors_fft_post, angle_errors_fft_phase_post,
      calibration_samples_sine, calibration_samples_cosine,
      calibration_samples_sine_scaled, calibration_samples_cosine_scaled,
      angleError, FFTAngleErrorMagnitude, FFTAngleErrorPhase, correctedError,
      FFTCorrectedErrorMagnitude, FFTCorrectedErrorPhase;

  enum Channel { ROTATION, ANGLE, COUNT, TEMPERATURE, CHANNEL_COUNT };

  enum Device { ADMT4000, TMC5240, DEVICE_COUNT };
  ...
  enum MotorRampMode { POSITION, RAMP_MODE_1 };
  ...
  const char *DeviceAttributes[DEVICE_ATTR_COUNT] = {
      "page",       "sequencer_mode", "angle_filt",     "conversion_mode",
      "h8_ctrl",    "sdp_gpio_ctrl",  "sdp_gpio0_busy", "sdp_coil_rs",
      "regmap_dump"};
  const char *MotorAttributes[MOTOR_ATTR_COUNT] = {
      "amax",       "rotate_vmax", "dmax",     "disable",
      "target_pos", "current_pos", "ramp_mode"};
  const uint32_t ConfigurationRegisters[CONFIGURATION_REGISTER_COUNT] = {
      0x01, 0x04, 0x06, 0x10, 0x12, 0x13, 0x1D, 0x23};
  const uint32_t ConfigurationPages[CONFIGURATION_REGISTER_COUNT] = {
      UINT32_MAX, UINT32_MAX, UINT32_MAX, 0x02, 0x02, 0x02, 0x02, 0x02};
  const uint32_t UniqueIdRegisters[UNIQID_REGISTER_COUNT] = {0x1E, 0x1F, 0x20,
                                                             0x21};
  const uint32_t UniqueIdPages[UNIQID_REGISTER_COUNT] = {0x02, 0x02, 0x02,
                                                         0x02};
  const uint32_t HarmonicRegisters[HARMONIC_REGISTER_COUNT] = {
      0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C};
  const uint32_t HarmonicPages[HARMONIC_REGISTER_COUNT] = {
      0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02};
  const uint32_t SensorRegisters[SENSOR_REGISTER_COUNT] = {
      0x03, 0x05, 0x08, 0x10, 0x11, 0x12, 0x13,
      0x18, 0x1D, 0x1E, 0x20, 0x23, 0x14};
  const uint32_t SensorPages[SENSOR_REGISTER_COUNT] = {
      UINT32_MAX, UINT32_MAX, UINT32_MAX, 0x00, 0x00, 0x00, 0x00,
      0x00,       0x00,       0x00,       0x00, 0x00, 0x02};

  const uint32_t RampGeneratorDriverFeatureControlRegisters
      [RAMP_GENERATOR_DRIVER_FEATURE_CONTROL_REGISTER_COUNT] = {0x33, 0x34,
                                                                0x35, 0x36};
  ...
  const uint32_t getRampGeneratorDriverFeatureControlRegister(
      RampGeneratorDriverFeatureControlRegister registerID);

  void connectADMT();
  void disconnectADMT();
  int getChannelIndex(const char *deviceName, const char *channelName);
  double getChannelValue(const char *deviceName, const char *channelName,
                         int bufferSize = 1);
  int getDeviceAttributeValue(const char *deviceName, const char *attributeName,
                              double *returnValue);
  int getDeviceAttributeValueString(const char *deviceName,
                                    const char *attributeName,
                                    char *returnValue, size_t byteLength = 512);
  int setDeviceAttributeValue(const char *deviceName, const char *attributeName,
                              double writeValue);
  QString calibrate(vector<double> PANG, int cycles, int samplesPerCycle,
                    bool CCW);
  int writeDeviceRegistry(const char *deviceName, uint32_t address,
                          uint32_t value);
  int readDeviceRegistry(const char *deviceName, uint32_t address,
                         uint32_t *returnValue);
  void computeSineCosineOfAngles(const vector<double> &angles);
  uint16_t calculateHarmonicCoefficientMagnitude(uint16_t harmonicCoefficient,
                                                 uint16_t originalValue,
                                                 const string &key);
  uint16_t calculateHarmonicCoefficientPhase(uint16_t harmonicPhase,
                                             uint16_t originalValue);
  double getActualHarmonicRegisterValue(uint16_t registerValue,
                                        const string key);
  ...
  uint16_t setGeneralRegisterBitMapping(uint16_t currentRegisterValue,
  ...
  uint16_t setDIGIOENRegisterBitMapping(uint16_t currentRegisterValue,
                                        map<string, bool> settings);
  uint16_t setDIGIORegisterBitMapping(uint16_t currentRegisterValue,
                                      map<string, bool> settings);
  ...
  map<string, double> getTmp1RegisterBitMapping(uint16_t registerValue,
                                                bool is5V);
...
private:
  ...
  template <typename Iter_T> void fft(Iter_T a, Iter_T b, int log2n);
  void performFFT(const vector<double> &angle_errors,
                  vector<double> &angle_errors_fft,
                  vector<double> &angle_errors_fft_phase, int cycleCount);
  int linear_fit(vector<double> x, vector<double> y, double *slope,
                 double *intercept);
  int calculate_angle_error(vector<double> angle_meas,
                            vector<double> &angle_error_ret,
                            double *max_angle_err);
  void getPreCalibrationFFT(const vector<double> &PANG,
                            vector<double> &angle_errors_fft_pre,
                            vector<double> &angle_errors_fft_phase_pre,
                            int cycleCount, int samplesPerCycle);
  void getPostCalibrationFFT(const vector<double> &updated_PANG,
                             vector<double> &angle_errors_fft_post,
                             vector<double> &angle_errors_fft_phase_post,
                             int cycleCount, int samplesPerCycle);
};
} // namespace scopy::admt
