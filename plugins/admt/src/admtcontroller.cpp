...
ADMTController::ADMTController(QString uri, QObject *parent)
  ...
  connect(this, &ADMTController::streamData, this,
          &ADMTController::handleStreamData);
  connect(this, &ADMTController::streamBufferedData, this,
          &ADMTController::handleStreamBufferedData);
...
const uint32_t
ADMTController::getHarmonicRegister(HarmonicRegister registerID) {
...
const uint32_t
ADMTController::getConfigurationRegister(ConfigurationRegister registerID) {
...
const uint32_t
ADMTController::getConfigurationPage(ConfigurationRegister registerID) {
  if (registerID >= 0 && registerID < CONFIGURATION_REGISTER_COUNT) {
...
const uint32_t
ADMTController::getUniqueIdRegister(UniqueIDRegister registerID) {
...
const uint32_t ADMTController::getRampGeneratorDriverFeatureControlRegister(
    RampGeneratorDriverFeatureControlRegister registerID) {
  if (registerID >= 0 &&
      registerID < RAMP_GENERATOR_DRIVER_FEATURE_CONTROL_REGISTER_COUNT) {
    return RampGeneratorDriverFeatureControlRegisters[registerID];
  }
  return UINT32_MAX;
}

int ADMTController::getChannelIndex(const char *deviceName,
...
double ADMTController::getChannelValue(const char *deviceName,
                                       const char *channelName,
                                       int bufferSize) {
  if (!m_iioCtx) {
  ...
  if (deviceCount < 1)
    return static_cast<double>(
        UINT64_MAX); // return QString("No devices found");

  iio_device *admtDevice = iio_context_find_device(m_iioCtx, deviceName);
  if (admtDevice == NULL)
    return static_cast<double>(
        UINT64_MAX); // return QString("No ADMT4000 device");

  int channelCount = iio_device_get_channels_count(admtDevice);
  if (channelCount < 1)
    return static_cast<double>(
        UINT64_MAX); // return QString("No channels found.");
  ...
  for (int i = 0; i < channelCount; i++) {
    ...
    if (deviceChannel != nullptr &&
        std::string(deviceChannel) == std::string(channelName)) {
      message = message + "[" + std::to_string(i) + "]" +
                std::string(deviceChannel) + ", ";
      break;
    } else {
  ...
  if (channel == NULL)
    return static_cast<double>(
        UINT64_MAX); // return QString("Channel not found.");
  ...
  iio_buffer *iioBuffer =
      iio_device_create_buffer(admtDevice, bufferSize, false);
  if (iioBuffer == NULL)
    return static_cast<double>(
        UINT64_MAX); // return QString("Cannot create buffer.");
  ...
  if (numBytesRead < 0)
    return static_cast<double>(
        UINT64_MAX); // return QString("Cannot refill buffer.");
...
int ADMTController::getDeviceAttributeValue(const char *deviceName,
...
int ADMTController::getDeviceAttributeValueString(const char *deviceName,
                                                  const char *attributeName,
                                                  char *returnValue,
                                                  size_t byteLength) {
  if (!m_iioCtx) {
  ...
  result =
      iio_device_attr_read(iioDevice, attributeName, returnValue, byteLength);
 ...
 * @return On success, 0 is returned.
 * @return On error, -1 is returned. */
int ADMTController::setDeviceAttributeValue(const char *deviceName,
                                            const char *attributeName,
                                            double writeValue) {
  if (!m_iioCtx) {
    return -1;
...
int ADMTController::writeDeviceRegistry(const char *deviceName,
...
int ADMTController::readDeviceRegistry(const char *deviceName, uint32_t address,
 ...
 * fitting) */
int ADMTController::linear_fit(vector<double> x, vector<double> y,
  ...
  *slope =
      (x.size() * sum_xy - sum_x * sum_y) / (x.size() * sum_x2 - sum_x * sum_x);

  *intercept =
      (sum_y * sum_x2 - sum_x * sum_xy) / (x.size() * sum_x2 - sum_x * sum_x);

  return 0;
}

int ADMTController::calculate_angle_error(vector<double> angle_meas,
                                          vector<double> &angle_error_ret,
                                          double *max_angle_err) {
  vector<double> angle_meas_rad(angle_meas.size()); // radian converted input
  vector<double> angle_meas_rad_unwrap(
      angle_meas.size()); // unwrapped radian input
  vector<double> angle_fit(
      angle_meas.size()); // array for polynomial fitted data
  ...
  for (int i = 1; i < angle_meas_rad.size(); i++) {
    ...
    double num3 = abs(angle_meas_rad[i] + num - angle_meas_rad_unwrap[i - 1] +
                      M_PI * 2.0);
    double num4 = abs(angle_meas_rad[i] + num - angle_meas_rad_unwrap[i - 1] -
                      M_PI * 2.0);
...
QString ADMTController::calibrate(vector<double> PANG, int cycleCount,
                                  int samplesPerCycle, bool CCW) {
  int circshiftData = 0;
  QString result = "";
  ...
  HAR_PHASE_1 =
      (int)(H1PHcor / pha_scale_factor_12bit) & (0xFFF); // 12bit number
  HAR_PHASE_2 =
      (int)(H2PHcor / pha_scale_factor_12bit) & (0xFFF); // 12bit number
  HAR_PHASE_3 =
      (int)(H3PHcor / pha_scale_factor_12bit) & (0xFFF); // 12bit number
  HAR_PHASE_8 =
      (int)(H8PHcor / pha_scale_factor_12bit) & (0xFFF); // 12bit number
...
void ADMTController::getPreCalibrationFFT(
    const vector<double> &PANG, vector<double> &angle_errors_fft_pre,
...
void ADMTController::postcalibrate(vector<double> PANG, int cycleCount,
                                   int samplesPerCycle, bool CCW) {
  int circshiftData = 0;
  QString result = "";
  ...
  getPostCalibrationFFT(PANG, angle_errors_fft_post,
                        angle_errors_fft_phase_post, cycleCount,
                        samplesPerCycle);
}

void ADMTController::getPostCalibrationFFT(
    const vector<double> &updated_PANG, vector<double> &angle_errors_fft_post,
  ...
  performFFT(angle_errors_post, angle_errors_fft_post,
             angle_errors_fft_phase_post, cycleCount);
...
void ADMTController::performFFT(const vector<double> &angle_errors,
  ...
  int N = pow(
      2, ceil(log2(L))); // Ensure size is a power of 2 (padding if necessary)
  ...
  for (int i = 0; i < N; i++) {
    ...
    angle_errors_fft_phase_temp[i] =
        atan2(fft_out[i].imag(), fft_out[i].real());
  }
...
void ADMTController::computeSineCosineOfAngles(const vector<double> &angles) {
  ...
  for (size_t i = 0; i < angles.size(); ++i) {
    ...
    calibration_samples_sine_scaled[i] =
        ((calibration_samples_sine[i] + 1) / 2) * (scaleMax - scaleMin) +
        scaleMin;
    calibration_samples_cosine_scaled[i] =
        ((calibration_samples_cosine[i] + 1) / 2) * (scaleMax - scaleMin) +
        scaleMin;
  }
}
...
// register
uint16_t ADMTController::calculateHarmonicCoefficientMagnitude(
    uint16_t harmonicCoefficient, uint16_t originalValue, const string &key) {
  uint16_t result = 0;
...
uint16_t
ADMTController::calculateHarmonicCoefficientPhase(uint16_t harmonicPhase,
...
double ADMTController::getActualHarmonicRegisterValue(uint16_t registerValue,
                                                      const string key) {
  double result = 0.0;
  ...
  if (key == "h1mag" || key == "h2mag") {
    ...
    // Convert the extracted value by applying CORDIC scaler and LSB
    result = extractedValue * LSB / cordicScaler;
  } else if (key == "h1phase" || key == "h2phase" || key == "h3phase" ||
             key == "h8phase") {
...
map<string, bool>
ADMTController::getFaultRegisterBitMapping(uint16_t registerValue) {
...
map<string, int>
ADMTController::getGeneralRegisterBitMapping(uint16_t registerValue) {
  map<string, int> result;

  // Bit 15: STORAGE[7]
  result["STORAGE[7]"] =
      ((registerValue >> 15) & 0x01) ? 1 : 0; // ? "Set" : "Not Set";
  ...
  // Bit 12: Angle Filter
  result["Angle Filter"] =
      ((registerValue >> 12) & 0x01) ? 1 : 0; // ? "Enabled" : "Disabled";

  // Bit 11: STORAGE[6]
  result["STORAGE[6]"] =
      ((registerValue >> 11) & 0x01) ? 1 : 0; // ? "Set" : "Not Set";

  // Bit 10: 8th Harmonic
  result["8th Harmonic"] =
      ((registerValue >> 10) & 0x01)
          ? 1
          : 0; // ? "User-Supplied Values" : "ADI Factory Values";
  ...
  result["Conversion Type"] =
      (registerValue & 0x01)
          ? 1
          : 0; // ? "One-shot conversion" : "Continuous conversions";
...
map<string, bool>
ADMTController::getDIGIOENRegisterBitMapping(uint16_t registerValue) {
  map<string, bool> result;
  ...
  // Bit 13: DIGIO5EN
  result["DIGIO5EN"] =
      ((registerValue >> 13) & 0x01)
          ? true
          : false; // ? "GPIO5 output enable" : "GPIO5 output disable";

  // Bit 12: DIGIO4EN
  result["DIGIO4EN"] =
      ((registerValue >> 12) & 0x01)
          ? true
          : false; // ? "GPIO4 output enable" : "GPIO4 output disable";

  // Bit 11: DIGIO3EN
  result["DIGIO3EN"] =
      ((registerValue >> 11) & 0x01)
          ? true
          : false; // ? "GPIO3 output enable" : "GPIO3 output disable";

  // Bit 10: DIGIO2EN
  result["DIGIO2EN"] =
      ((registerValue >> 10) & 0x01)
          ? true
          : false; // ? "GPIO2 output enable" : "GPIO2 output disable";

  // Bit 9: DIGIO1EN
  result["DIGIO1EN"] =
      ((registerValue >> 9) & 0x01)
          ? true
          : false; // ? "GPIO1 output enable" : "GPIO1 output disable";

  // Bit 8: DIGIO0EN
  result["DIGIO0EN"] =
      ((registerValue >> 8) & 0x01)
          ? true
          : false; // ? "GPIO0 output enable" : "GPIO0 output disable";

  // // Bits 7:6: Reserved (skipped)
  // result["Reserved (7:6)"] = "Reserved";

  // Bit 5: Bootload
  result["BOOTLOAD"] = ((registerValue >> 5) & 0x01)
                           ? true
                           : false; // ? "GPIO5" : "Bootload (Output only)";

  // Bit 4: Fault
  result["FAULT"] = ((registerValue >> 4) & 0x01)
                        ? true
                        : false; // ? "GPIO4" : "Fault (Output only)";

  // Bit 3: Acalc
  result["ACALC"] = ((registerValue >> 3) & 0x01)
                        ? true
                        : false; // ? "GPIO3" : "Acalc (Output only)";

  // Bit 2: Sent
  result["SENT"] = ((registerValue >> 2) & 0x01)
                       ? true
                       : false; // ? "GPIO2" : "Sent (Output only)";

  // Bit 1: Cnv
  result["CNV"] = ((registerValue >> 1) & 0x01)
                      ? true
                      : false; // ? "GPIO1" : "Cnv (Output only)";

  // Bit 0: Busy
  result["BUSY"] = (registerValue & 0x01)
                       ? true
                       : false; // ? "GPIO0" : "Busy (Output only)";

  return result;
}

map<string, bool>
ADMTController::getDIGIORegisterBitMapping(uint16_t registerValue) {
  map<string, bool> result;
...
map<string, bool>
ADMTController::getDiag1RegisterBitMapping_Register(uint16_t registerValue) {
  map<string, bool> result;
...
map<string, double>
ADMTController::getDiag1RegisterBitMapping_Afe(uint16_t registerValue,
                                               bool is5V) {
  map<string, double> result;

  // Bits 7:0: AFE Diagnostic 2 - Measurement of Fixed voltage (stored in 2's
  // complement)
  int8_t afeDiagnostic =
      static_cast<int8_t>(registerValue & 0x00FF); // Interpret as signed 8-bit

  // Choose the correct resolution based on the voltage level (5V or 3.3V part)
  double resolution =
      is5V ? 0.0048828 : 0.003222; // 0.0048828 for 5V, 0.003222 for 3.3V
...
map<string, double>
ADMTController::getDiag2RegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  double diagnostic1Voltage = static_cast<double>(afeDiagnostic1) *
                              0.01; // what I found (to be confirmed)
  ...
  // Rain: adjust scaling factor as needed with actual method.
  double diagnostic0Voltage = static_cast<double>(afeDiagnostic0) *
                              0.01; // what I found (to be confirmed)
...
uint16_t
ADMTController::setGeneralRegisterBitMapping(uint16_t currentRegisterValue,
                                             map<string, int> settings) {
  uint16_t registerValue =
      currentRegisterValue; // Start with the current register value
...
int ADMTController::getAbsAngleTurnCount(uint16_t registerValue) {
  ...
  if (turnCount <= 0xD5) {
    ...
    int8_t signedTurnCount =
        static_cast<int8_t>(turnCount); // Handle as signed value
    return signedTurnCount / 4; // Convert from quarter turns to whole turns
  }
}

uint16_t
ADMTController::setDIGIOENRegisterBitMapping(uint16_t currentRegisterValue,
...
uint16_t
ADMTController::setDIGIORegisterBitMapping(uint16_t currentRegisterValue,
                                           map<string, bool> settings) {
  uint16_t registerValue =
      currentRegisterValue; // Start with the current register value
...
map<string, string>
ADMTController::getUNIQID3RegisterMapping(uint16_t registerValue) {
  ...
  uint8_t asilID =
      (registerValue >> 3) & 0x07; // Show both Seq 1 & 2 if unknown
  switch (asilID) {
  case 0x00:
...
map<string, double>
ADMTController::getSineRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  int16_t sineValueRaw =
      (registerValue >> 2); // Shift right by 2 to discard Bits [1:0]
...
map<string, double>
ADMTController::getCosineRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
...
map<string, double>
ADMTController::getRadiusRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  uint16_t radiusRaw =
      (registerValue >> 1); // Shift right by 1 to discard Bit 0
...
map<string, double>
ADMTController::getAngleSecRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  uint16_t angleSecRaw =
      (registerValue >> 4); // Right-shift by 4 to discard Bits [3:0]
...
map<string, double>
ADMTController::getSecAnglQRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  int16_t secAnglQRaw = static_cast<int16_t>(
      (registerValue & 0xFFFC) >> 2); // Mask Bits [1:0] and shift right by 2
...
map<string, double>
ADMTController::getSecAnglIRegisterBitMapping(uint16_t registerValue) {
  map<string, double> result;
  ...
  int16_t secAnglIRaw = static_cast<int16_t>(
      (registerValue & 0xFFFC) >> 2); // Mask Bits [1:0] and shift right by 2
...
map<string, double>
ADMTController::getTmp1RegisterBitMapping(uint16_t registerValue, bool is5V) {
  map<string, double> result;
...
int ADMTController::streamIO() {
  ...
  struct iio_device *admtDevice =
      iio_context_find_device(m_iioCtx, deviceName); // Find the ADMT device
  if (admtDevice == NULL)
    return result;
  struct iio_channel *channel = iio_device_find_channel(
      admtDevice, channelName, isOutput); // Find the rotation channel
  if (channel == NULL)
    return result;
  iio_channel_enable(channel); // Enable the channel
  int scaleRet = iio_channel_attr_read_double(
      channel, scaleAttrName, scaleAttrValue); // Read the scale attribute
  if (scaleRet != 0)
    return scaleRet;
  ...
  struct iio_buffer *buffer = iio_device_create_buffer(
      admtDevice, samples, isCyclic); // Create a buffer
  ...
  while (!stopStream) {
    ...
    for (pointerData = static_cast<char *>(iio_buffer_first(buffer, channel));
...
void ADMTController::bufferedStreamIO(int totalSamples, int targetSampleRate) {
  ...
  while (!stopStream && sampleCount < totalSamples) {
    ...
    for (pointerData = static_cast<char *>(iio_buffer_first(buffer, channel));
      ...
      for (j = 0; j < repeat; j++) {
        if (format->length / 8 == sizeof(int16_t)) {
          rawBufferedValues.push_back(
              (reinterpret_cast<int16_t *>(pointerData))[j]);
          sampleCount++;
          continue;
        }
      }
  ...
  for (int i = 0; i < rawBufferedValues.size(); i++) {
    double scaledValue =
        (rawBufferedValues[i] - offsetAttrValue) * *scaleAttrValue;
...
void ADMTController::handleStreamBufferedDataInterval(
    const QVector<uint32_t> &value) {
