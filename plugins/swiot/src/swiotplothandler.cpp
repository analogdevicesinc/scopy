...
#include <QGridLayout>
#include "captureplot/filemanager.h"
...
SwiotPlotHandler::SwiotPlotHandler(QWidget *parent, int plotChnlsNo) :
	QWidget(parent)
{
	m_lock = new QMutex();
...
SwiotPlotHandler::~SwiotPlotHandler()
{
...
void SwiotPlotHandler::initPlot(int plotChnlsNo)
{
...
void SwiotPlotHandler::onBufferRefilled(QVector<QVector<double>> bufferData, int bufferCounter)
{
...
void SwiotPlotHandler::onPlotChnlsChanges(std::vector<bool> enabledPlots)
{
...
void SwiotPlotHandler::onBtnExportClicked(QMap<int, bool> exportConfig)
{
//	pause(true);
	QStringList filter;
...
void SwiotPlotHandler::setPlotActiveAxis(int id) {
	m_plot->setActiveVertAxis(id, true);
}

QColor SwiotPlotHandler::getCurveColor(int id) const
{
	return m_plot->getLineColor(id);
}

QWidget *SwiotPlotHandler::getPlotWidget() const
{
	return m_plotWidget;
}

void SwiotPlotHandler::resetPlot()
{
...
void SwiotPlotHandler::onTimespanChanged(double value)
{
	m_timespan = value;
	resetPlot();
}

void SwiotPlotHandler::resetDataPoints()
{
	if (m_dataPoints.size() > 0) {
		for (int i = 0; i < m_dataPoints.size(); i++) {
			delete[] m_dataPoints[i];
