...
BufferReaderThread::BufferReaderThread(struct iio_device *iioDev) :
	m_enabledChnlsNo(0)
      ,m_iioDev(iioDev)
      ,m_iioBuff(nullptr)
      ,m_offsetScaleValues()
...
double BufferReaderThread::convertData(unsigned int data, int idx)
{
...
void BufferReaderThread::enableIioChnls()
{
	if (m_iioDev) {
...
QVector<std::pair<double, double>> BufferReaderThread::getOffsetScaleVector()
{
...
int BufferReaderThread::getEnabledChnls()
{
	int enChnls = 0;
	for (const auto &key : m_chnlsInfo.keys()) {
...
void BufferReaderThread::createIioBuffer()
{
...
void BufferReaderThread::destroyIioBuffer()
{
	if (m_iioBuff) {
...
void BufferReaderThread::onChnlsChange(QMap<int, struct chnlInfo*> chnlsInfo)
{
	m_chnlsInfo = chnlsInfo;
}

void BufferReaderThread::run()
{
