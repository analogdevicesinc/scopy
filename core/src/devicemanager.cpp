...
void DeviceManager::disconnectingDevice(QString id) {
	Q_EMIT deviceDisconnecting(id);
}
...
bool DeviceManager::busy() {
	...
	for(auto dev : map) {
		if(dev->state() == Device::DEV_ERROR || dev->state() == Device::DEV_INIT || dev->state() == Device::DEV_CONNECTING || dev->state() == Device::DEV_DISCONNECTING)
			return true;
	}
	return false;
}

int DeviceManager::connectedDeviceCount() {
	return connectedDev.size();
}
