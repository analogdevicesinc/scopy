...
if (switchToTool("Rms")) {
    ...
    TestFramework.runTest("TST.PQM.RMS_OPEN", function() {
        try {
            ...
            var loggingEnabled = pqm.isRmsLoggingEnabled();
...
if (switchToTool("Settings")) {
    ...
    TestFramework.runTest("TST.PQM.SETTINGS_MULTI_ATTR", function() {
        try {
            pqm.settingsRead();
            msleep(2000);
            var attributes = ["msv_carrier_frequency", "nominal_voltage", "nominal_frequency"];
            for (var i = 0; i < attributes.length; i++) {
                var value = pqm.getSettingsAttributeValue(attributes[i]);
                // Just verify we can read the attributes without error
            }
            return true;
        } catch (e) {
            printToConsole("  Error: " + e);
