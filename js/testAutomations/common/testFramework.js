...
var TestFramework = {
    ...
    deviceUri: "ip:192.168.2.1",  // Default device IP, can be overridden
    deviceId: null,
    isConnected: false,
    ...
    connectToDevice: function(uri) {
        ...
        try {
            ...
            if (this.deviceId) {
                try { scopy.removeDevice(this.deviceUri); } catch (e2) {}
            }
        }

        // Fallback to emulator (ip:127.0.0.1)
        printToConsole("Device not found, trying emulator...");
    ...
    disconnectFromDevice: function() {
        if (this.isConnected && this.deviceId) {
            try {
                ...
                scopy.removeDevice(this.deviceUri);
                this.isConnected = false;
