...
TestFramework.runTest("TST.AD936X.PLUGIN_LOADS", function() {
    try {
        ...
        if (!TestFramework.supervisedCheck("Verify AD936x plugin loads and is accessible in the UI")) return false;
...
TestFramework.runTest("TST.AD936X.DEVICE_DETECTION_AND_DISPLAY", function() {
    try {
        ...
        if (!TestFramework.supervisedCheck("Verify device status panels are displayed correctly and ENSM mode, RX/TX bandwidth values match the UI")) return false;
...
TestFramework.runTest("TST.AD936X.CHANGE_VALIDATE_GLOBAL_SETTINGS.VISUAL", function() {
    try {
        ...
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify ENSM mode changes to FDD in the UI")) return false;

        // Step 2: Change rate governor
        printToConsole("  Setting TRX rate governor to nominal...");
        ad936x.setTrxRateGovernor("nominal");
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify rate governor changes in the UI")) return false;
...
TestFramework.runTest("TST.AD936X.RX_TX_CHAIN_CONFIG.VISUAL", function() {
    try {
        ...
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify RX bandwidth updated in UI")) return false;

        printToConsole("  Setting RX sampling frequency to 25 MHz...");
        ad936x.setRxSamplingFrequency("25000000");
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify RX sampling frequency updated in UI")) return false;
        ...
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify TX bandwidth updated in UI")) return false;

        printToConsole("  Setting TX LO frequency to 2.4 GHz...");
        ad936x.setTxLoFrequency("2400000000");
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify TX LO frequency updated in UI")) return false;
...
TestFramework.runTest("TST.AD936X_ADVANCED.PLUGIN_DETECTION_AND_DISPLAY.VISUAL", function() {
    try {
        ...
        if (!TestFramework.supervisedCheck("Verify AD936x Advanced plugin is loaded")) return false;
        ...
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify RSSI panel displays correctly")) return false;

        ad936x_advanced.switchSubtab("TX Monitor");
        msleep(500);
        if (!TestFramework.supervisedCheck("Verify TX Monitor panel displays correctly")) return false;
