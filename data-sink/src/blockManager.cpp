...
BlockManager::BlockManager(bool waitforAllSources)
    : QObject()
...
int BlockManager::addOutputLink(SourceBlock *source, BasicBlock *final, uint final_ch, uint out_ch)
{
	...
	[=](ChannelData *data, uint block_ch) {
               ...
               bool threaded)
{
    ...
    if(m_blockLinks.contains(source)) {
	    ...
	    [=](uint ch, bool en) {
                ...
                // if running and a ch from a source with no enabled channels is enabled, start the request cycle again
...
bool BlockManager::start(bool cyclic)
{
    m_running = cyclic;
