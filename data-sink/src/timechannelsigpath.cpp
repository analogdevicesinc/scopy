...
void OffsetFilter::setOffset(double offset)
{
	m_offset = offset;
}

void OffsetFilter::setScale(double scale)
{
	m_scale = scale;
}

double OffsetFilter::getOffset() const
{
	return m_offset;
}

double OffsetFilter::getScale() const
{
	return m_scale;
}
...
ChannelDataVector* OffsetFilter::createData()
{
	...
	std::transform(inputData->data.begin(), inputData->data.end(),
		       ...
		       [this](float value) {
			       return (value * static_cast<float>(m_scale)) + static_cast<float>(m_offset);
		       });
...
void SignalPath::setManager(BlockManager *manager)
{
	m_manager = manager;
}

BlockManager *SignalPath::manager() const
{
	return m_manager;
}

QString SignalPath::name() const
{
	return m_name;
}
...
void SignalPath::removeFilter(FilterBlock *filter)
{
	m_filters.removeAll(filter);
}

QList<FilterBlock*> SignalPath::filters() const
{
	return m_filters;
}

// TimeChannelSigpath implementation
TimeChannelSigpath::TimeChannelSigpath(QString name, ChannelComponent *ch, SourceBlock *sourceBlock,
				       uint sourceChannel, uint outputChannel, BlockManager *manager, QObject *parent)
	: QObject(parent)
	, m_name(name)
	, m_ch(ch)
...
bool TimeChannelSigpath::isEnabled() const
{
	return m_enabled;
}
...
double TimeChannelSigpath::getOffset() const
{
	return m_offsetFilter ? m_offsetFilter->getOffset() : 0.0;
}

double TimeChannelSigpath::getScale() const
{
	return m_offsetFilter ? m_offsetFilter->getScale() : 1.0;
}

SignalPath *TimeChannelSigpath::sigpath()
{
	return m_signalPath;
}

ChannelComponent *TimeChannelSigpath::channelComponent() const
{
	return m_ch;
}

SourceBlock *TimeChannelSigpath::sourceBlock() const
{
	return m_sourceBlock;
}

uint TimeChannelSigpath::sourceChannel() const
{
	return m_sourceChannel;
}

uint TimeChannelSigpath::outputChannel() const
{
