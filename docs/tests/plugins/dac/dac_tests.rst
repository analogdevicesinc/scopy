...
Description:
    This test verifies that the DAC plugin is compatible with the selected device
...
Steps:
    ...
    2. Switch the "Mode" to "Buffer" and check the "Channels" list:
        - **Expected result:** The channels list contains the following: **cf-ad9361-dds-core-lpc:voltage0** and
          **cf-ad9361-dds-core-lpc:voltage1**.
...
Description:
    This test verifies that the DAC plugin is able to load a file in buffer mode.
...
Steps:
    ...
    3. Select the *sine_complex_1_15360_sr_15360000_samps_40000_ampl_32767.csv* file using the "Browse" button.
    ...
    5. Select the json file from resources file using the "Browse" button.
...
UID:
    TST.DAC.BUFFER_CHANNEL_SELECT

Description:
    This test verifies that the DAC plugin is able to validate channel selection in buffer mode.
...
UID:
    TST.DAC.BUFFER_FILE_SIZE

Description:
    This test verifies that the DAC plugin is able to validate the file size in buffer mode.
...
Steps:
    ...
    6. Change the "File Size" field to a value lower than the actual file size (write 50 and press Enter).
    ...
    8. Change the "File Size" using the "+" button.
        - **Expected result:** The field value changes to the current value +1.
...
UID:
    TST.DAC.BUFFER_CYCLIC

Description:
    This test verifies the DAC plugin buffer generation in cyclic mode.
...
Description:
    This test verifies the DAC plugin DDS modes.
...
Description:
    This test verifies the DAC plugin DDS mode attributes refresh.
...
Description:
    This test verifies the DAC plugin tutorial.
...
UID:
    TST.DAC.CHANNEL_ATTRS

Description:
    This test verifies the DAC plugin channel attributes.
